...
### **RGBSIFTChannelAverageWrapper** (`RGBSIFTChannelAverageWrapper.hpp/cpp`)
**Type**: Channel-averaged color SIFT
**Dimension**: 128
**Output Type**: CV_32F

**How it works**:
1. Computes SIFT descriptors on R, G, B channels independently
2. Averages the three 128-dim descriptors element-wise
3. Results in single 128-dim descriptor (same as grayscale SIFT)

**Key Advantage**:
- Incorporates color information without 3x memory overhead
- Same dimensionality as SIFT (easier comparison)
- Better than grayscale SIFT in color-rich scenes

**Usage**: Balance between color information and compact representation.
...
DescriptorType::RGBSIFT_CHANNEL_AVG → RGBSIFTChannelAverageWrapper
...
Results on HPatches with `sift_scale_matched_6px` keypoints (scale-filtered):

| Wrapper | Dimension | Type | mAP | Speed | Use Case |
|---------|-----------|------|-----|-------|----------|
| **SIFT** | 128 | Float | 63.86% | Fast | Grayscale baseline |
| **RootSIFT** | 128 | Float | 65.31% | Fast | L1+sqrt normalized |
| **DSPSIFT_V2** | 128 | Float | 65.31% | Medium | Multi-scale pooling |
| **RGBSIFT** | 384 | Float | 64.69% | Slow | Color scenes |
| **RGBSIFT_CHANNEL_AVG** | 128 | Float | 64.03% | Medium | Compact color |
| **DSPRGBSIFT_V2** | 384 | Float | 66.03% | Slow | Best SIFT-family |
| **HardNet (LibTorch)** | 128 | Float | 78.95%* | Slow | CNN descriptor |
| **SOSNet (LibTorch)** | 128 | Float | 78.79%* | Slow | CNN descriptor |
| **SURF** | 64-128 | Float | 65.21% | Fast | SIFT alternative |
| **ORB** | 32 bytes | Binary | N/A | Very Fast | Real-time/binary |

*CNN results on `keynet_scale_matched_6px` keypoints (KeyNet detector)
...
3. **Add to `src/include/thesis_project/types.hpp`**:
   ```cpp
