...
int main(int argc, char** argv) {
    ...
    if (command == "import-csv") {
        ...
        // Usage: ./keypoint_manager generate-random-subset <source_set> <target_count> <output_name> [--seed N]
        if (argc < 5) {
        ...
        // Note: We'll check total count during loading

        // Check if output set already exists
        if (db.getKeypointSetInfo(output_set_name)) {
            std::cerr << "❌ Output keypoint set already exists: " + output_set_name << std::endl;
            std::cerr << "   Use --overwrite flag to replace existing set" << std::endl;
            return 1;
        }
        ...
        // 1. Get all keypoint IDs from source set
        // 2. Shuffle using std::mt19937 with seed
        // 3. Take first target_count IDs
        // 4. Load those keypoints and store in new set
        ...
        if (all_ids.size() < target_count) {
            std::cerr << "❌ Source set only has " << all_ids.size()
        ...
        // Load and store selected keypoints
        ...
        // Usage: ./keypoint_manager generate-top-n <source_set> <target_count> --by <property> --out <output_name>
        if (argc < 7) {
        ...
        // Group by scene/image and store
        ...
        // Usage: ./keypoint_manager generate-spatial-subset <source_set> <target_count> <min_distance> <output_name>
        if (argc < 6) {
        ...
        // Greedy spatial filtering per image:
        // 1. Sort keypoints by response (strongest first)
        // 2. Select keypoints maintaining minimum distance
        ...
        LOG_INFO("✅ Spatial subset generated successfully!");
        LOG_INFO("   Output set: " + output_set_name);
        ...
        if (total_stored < target_count) {
            LOG_WARNING("⚠️  Only stored " + std::to_string(total_stored) + " keypoints (target was " +
