#include "ORBKeypointGenerator.hpp"
#include <algorithm>
#include <cmath>

namespace thesis_project {

ORBKeypointGenerator::ORBKeypointGenerator(
    int num_features,
    float scale_factor,
    int num_levels,
    int edge_threshold,
...
std::vector<cv::KeyPoint> ORBKeypointGenerator::detect(
    const cv::Mat& image,
...
std::vector<cv::KeyPoint> ORBKeypointGenerator::detectNonOverlapping(
    const cv::Mat& image,
    float min_distance,
    const KeypointParams& params
) {
...
std::vector<cv::KeyPoint> ORBKeypointGenerator::applyBoundaryFilter(
...
std::vector<cv::KeyPoint> ORBKeypointGenerator::applyKeypointLimit(
    std::vector<cv::KeyPoint> keypoints,
    int max_keypoints
) const {
...
std::vector<cv::KeyPoint> ORBKeypointGenerator::filterOverlapping(
    std::vector<cv::KeyPoint> keypoints,
    float min_distance
) const {
