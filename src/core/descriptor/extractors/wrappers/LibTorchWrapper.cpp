...
namespace thesis_project::wrappers {
    ...
    cv::Mat LibTorchWrapper::extract(const cv::Mat& imageBgrOrGray,
        ...
        const torch::Tensor batch = torch::stack(patches, 0).to(target_device);
    ...
    torch::Tensor LibTorchWrapper::makePatch(const cv::Mat& imageGray, const cv::KeyPoint& kp) const {
        ...
        const float kpSize = std::max(kp.size, 1e-3f);  // Guard against zero/negative keypoint size
        const float scale_radius = kpSize / 2.0f;  // Convert diameter to radius (Kornia standard)
        // Scale to map radius to half the patch size (32/2 = 16 pixels for radius)
        const double scale = static_cast<double>(CNN_INPUT_SIZE) / (2.0 * static_cast<double>(scale_radius));

        // Determine rotation angle (undo keypoint angle to make patch upright)
        const float angle_deg = (rotate_upright_ && kp.angle >= 0.0f) ? -kp.angle : 0.0f;
        ...
        const double cx = CNN_INPUT_SIZE * 0.5;  // Patch center x
        const double cy = CNN_INPUT_SIZE * 0.5;  // Patch center y
